...
void pageRank(Graph g, double *solution, double damping, double convergence)
{
	...
	while(!converged)
	{
		double node_no_out_edge_sum = 0.0;

		#pragma omp parallel for reduction (+:node_no_out_edge_sum)
		for(int i = 0;i < numNodes;i++)
		{
		...
		node_no_out_edge_sum = damping * node_no_out_edge_sum / numNodes;
		...
		for(int i = 0;i < numNodes;i++)
		{
			...
			{
				score_new += node_out_edge_give[*iter];
			}

			score_new = (damping * score_new) + (1.0 - damping) / numNodes + node_no_out_edge_sum;

			global_diff += abs(solution[i] - score_new);
